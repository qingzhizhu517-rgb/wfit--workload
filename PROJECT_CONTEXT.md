...
> **2026-09-10 简化为两级**：院领导签字环节已移除，旧状态 3 迁移为 2
> （`sql/17_simplify_approval.sql`）。

```
0 填报中 ──提交──▶ 1 教务处待审 ──审核通过(即终态,写 lock_time)──▶ 2 已完结(锁定)
   ▲                    │
   └────────驳回────────┘        管理员解锁：2 → 0
```

驳回**不退化为 -1**，直接回到 0（`-1` 从未实现，是旧文档的说法）。

审计落在 `biz_audit_log`（from_status/to_status 为 TINYINT(1)；
简化的存量日志中仍可能有历史值 3 与 action=`sign`）。

**明细表 `biz_workload_item.status` 是另一套状态机**：0草稿/1已核对/2有异议/3已驳回，
与汇总状态无关，勿混改。
