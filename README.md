...
本系统替代教师手动汇总、核对、填写工作量的传统模式，实现教务处→学院→教师的全流程数字化管理。支持 G1-G11 共 11 类工作量的自动核算、学期汇总、绩效酬金计算、两级审批和报表导出。
...
| G11 | 管理服务工作量 | 直接同步教务核定的本学期岗位减免值；学期汇总封顶 180 |
...
- **审批（两级）**：填报中 → 教务处待审 → 已完结（2026-09-10 简化，原院领导签字环节已移除）
...
| 22_semester_position_workload | 新库已并入 `01` | 已有开发库升级时补来源批次及 G11 来源+学期唯一键，可重复执行；仅改结构，不转换旧学年值、不重算历史记录 |
