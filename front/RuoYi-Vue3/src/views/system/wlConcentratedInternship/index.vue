<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['system:wlConcentratedInternship:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['system:wlConcentratedInternship:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['system:wlConcentratedInternship:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['system:wlConcentratedInternship:export']">导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-alert type="info" :closable="false" class="mb8" title="核算公式：工作量 = 实习周数 × 指导人数(上限20) × 每周常量（规则 CONST_COURSE_DESIGN，默认 0.4）" />

    <el-table v-loading="loading" :data="wlConcentratedInternshipList" stripe empty-text="暂无数据" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="50" align="center" />
      <el-table-column label="明细ID" align="center" prop="itemId" width="70" />
      <el-table-column label="实习周数" align="right" prop="W" width="100">
        <template #default="scope">
          <span class="coef-main">{{ formatNumber(scope.row.W) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="指导人数" align="right" prop="R6" width="100">
        <template #default="scope">{{ formatNumber(scope.row.R6, 0) }}</template>
      </el-table-column>
    ...
    <el-dialog :title="title" v-model="open" width="520px" append-to-body>
      <el-form ref="wlConcentratedInternshipRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="实习周数" prop="W">
          <el-input-number v-model="form.W" :min="0" :precision="2" controls-position="right" style="width: 100%" />
        </el-form-item>
        <el-form-item label="指导人数" prop="R6">
          <el-input-number v-model="form.R6" :min="0" :max="99" :precision="0" controls-position="right" style="width: 100%" />
