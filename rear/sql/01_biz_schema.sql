...
CREATE TABLE biz_teaching_task (
  ...
  is_retake            TINYINT(1)    NOT NULL DEFAULT 0          COMMENT '重修标志（不占层次列；课程名含「重修」时置1）',
...
CREATE TABLE biz_workload_item (
  ...
  -- TODO: 异议/申诉链路延后至下一迭代（dean_approval_* 字段暂保留不启用）
  ...
  -- TODO: 异议/申诉链路延后至下一迭代（appeal_* 字段暂保留不启用）
...
CREATE TABLE biz_wl_thesis (
  ...
  discipline_category VARCHAR(20) DEFAULT NULL    COMMENT '学科门类 SCITECH/LIBERAL_ARTS，附件1 X/Y 双列分列用；艺术类按文史',
  create_by VARCHAR(64) DEFAULT '', create_time DATETIME DEFAULT NULL,
  update_by VARCHAR(64) DEFAULT '', update_time DATETIME DEFAULT NULL,
  remark VARCHAR(500) DEFAULT NULL,
  PRIMARY KEY (item_id)
...
CREATE TABLE biz_workload_summary (
  ...
  g8_remark                VARCHAR(255)  DEFAULT NULL            COMMENT '其他工作量说明（附件1 AF 列）',
  g11_remark               VARCHAR(255)  DEFAULT NULL            COMMENT '管理服务工作量说明（附件1 AI 列）',
